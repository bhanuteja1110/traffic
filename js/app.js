(function() {
  ...
  const App = {
    ...
    updateDashboard(scenarioData, smartData) {
      ...
      if (delayBadge) {
        if (delay > 0) {
          delayBadge.innerHTML = `<i data-lucide="clock" style="width:14px; height:14px;"></i> +${delay} min slower than usual`;
    ...
    updateAnalysis(scenarioData, smartData) {
      ...
      if (delayResult) {
        if (scenarioData.eta > threshold) {
          delayResult.innerHTML = `<span class="delay-formula">${scenarioData.eta} > ${threshold}</span><span class="delay-verdict" style="color:var(--danger);"><i data-lucide="alert-triangle" style="width:16px;height:16px;margin-right:4px;"></i>Delay Detected</span>`;
        } else {
          delayResult.innerHTML = `<span class="delay-formula">${scenarioData.eta} < ${threshold}</span><span class="delay-verdict" style="color:var(--success);"><i data-lucide="check-circle" style="width:16px;height:16px;margin-right:4px;"></i>No Significant Delay</span>`;
        }
      }
      ...
      if (breakdownEl) {
        breakdownEl.innerHTML = `
          <div class="risk-breakdown-item">
            <span class="risk-factor-name" style="width:80px; text-align:left; display:flex; align-items:center; gap:6px;"><i data-lucide="car" style="width:14px;height:14px;"></i> Traffic</span>
          ...
          <div class="risk-breakdown-item">
            <span class="risk-factor-name" style="width:80px; text-align:left; display:flex; align-items:center; gap:6px;"><i data-lucide="hard-hat" style="width:14px;height:14px;"></i> Constr.</span>
          ...
          <div class="risk-breakdown-item">
            <span class="risk-factor-name" style="width:80px; text-align:left; display:flex; align-items:center; gap:6px;"><i data-lucide="cloud-rain" style="width:14px;height:14px;"></i> Weather</span>
          ...
          <div class="risk-breakdown-item">
            <span class="risk-factor-name" style="width:80px; text-align:left; display:flex; align-items:center; gap:6px;"><i data-lucide="users" style="width:14px;height:14px;"></i> Peak</span>
    ...
    saveRoute() {
      ...
      if (from && to) {
        this.showToast('<i data-lucide="check-circle" style="width:18px;height:18px;color:var(--success);"></i> Route added successfully!');
        this.hideModal();
      } else {
        this.showToast('<i data-lucide="alert-circle" style="width:18px;height:18px;color:var(--warning);"></i> Please fill all fields');
      }
    },
    
    saveTrip() {
      this.showToast('<i data-lucide="save" style="width:18px;height:18px;color:var(--success);"></i> Trip details saved!');
      setTimeout(() => {
        this.history = [];
        this.navigateTo('dashboard-screen', false);
        document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
